...
import 'dart:io';
import '../lib/features/discovery/data/datasources/project_gutenberg_data_source.dart';
import '../lib/features/discovery/data/datasources/internet_archive_data_source.dart';
import '../lib/features/discovery/data/datasources/openlibrary_data_source.dart';
import '../lib/features/discovery/data/repositories/project_gutenberg_repository_impl.dart';
import '../lib/features/discovery/data/repositories/internet_archive_repository_impl.dart';
import '../lib/features/discovery/data/repositories/openlibrary_repository_impl.dart';
...
void main() async {
  ...
  try {
    ...
    try {
      ...
      final pageResult = await olRepository.searchBooks(query: 'test', page: -1);
      pageResult.fold(
      ...
      final limitResult = await olRepository.searchBooks(query: 'test', limit: 200);
      limitResult.fold(
    ...
    try {
      ...
      olStatus.fold(
        (failure) => print('   📡 OpenLibrary status check implemented (${failure.runtimeType})'),
        (status) => print('   ✅ OpenLibrary status: ${status.isAvailable ? "Available" : "Unavailable"}'),
      );
      
      iaStatus.fold(
        (failure) => print('   📡 Internet Archive status check implemented (${failure.runtimeType})'),
        (status) => print('   ✅ Internet Archive status: ${status.isAvailable ? "Available" : "Unavailable"}'),
      );
