...
class PerformanceBenchmark {
  ...
  static PerformanceBenchmark get instance =>
      _instance ??= PerformanceBenchmark._();
  ...
  Future<BenchmarkTestResult> _benchmarkAppStartup() async {
    ...
    try {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        stopwatch.stop();
        tests['startup_attempt_${i + 1}'] =
            stopwatch.elapsedMilliseconds.toDouble();
      }

      final averageStartup =
          tests.values.reduce((a, b) => a + b) / tests.length;
  ...
  Future<void> saveBenchmarkResults(BenchmarkResult result) async {
    try {
      ...
      final fileName =
          'benchmark_${result.timestamp.millisecondsSinceEpoch}.json';
...
class BenchmarkResult {
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'test_results':
          testResults.map((key, value) => MapEntry(key, value.toJson())),
