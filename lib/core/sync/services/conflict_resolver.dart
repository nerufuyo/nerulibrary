...
class DefaultConflictResolver implements ConflictResolver {
    ...
    if (timeDifference.inSeconds <= 1) {
      ...
      if (_hasDataDifferences(localData, remoteData, entityType)) {
        return SyncConflict(
          ...
          entityId: localData['id']?.toString() ?? remoteData['id']?.toString() ?? 'unknown',
          localData: localData,
    ...
    try {
      switch (strategy) {
        ...
        case ConflictResolution.merge:
          return mergeData(conflict.localData, conflict.remoteData, conflict.entityType);
      ...
      return Left(SyncDataFailure(message: 'Failed to resolve conflict: ${e.toString()}'));
    }
  }
  ...
  ConflictResolution getRecommendedResolution(SyncConflict conflict) {
    ...
    if (conflict.entityType == 'book' || conflict.entityType == 'author' || conflict.entityType == 'category') {
  ...
  bool canResolveAutomatically(SyncConflict conflict) {
    ...
    final timeDifference = conflict.localUpdatedAt.difference(conflict.remoteUpdatedAt).abs();
  ...
  Either<SyncFailure, Map<String, dynamic>> mergeData(
    ...
    try {
      ...
      return Left(SyncDataFailure(message: 'Failed to merge data: ${e.toString()}'));
    }
  }
  ...
  bool _hasDataDifferences(
    ...
    final ignoreFields = ['updatedAt', 'updated_at', 'createdAt', 'created_at', 'lastSyncAt'];
  ...
  Either<SyncFailure, Map<String, dynamic>> _mergeReadingProgress(
    ...
    final localProgress = (local['progressPercentage'] as num?)?.toDouble() ?? 0.0;
    final remoteProgress = (remote['progressPercentage'] as num?)?.toDouble() ?? 0.0;
  ...
  Either<SyncFailure, Map<String, dynamic>> _mergeCollection(
    ...
    if (remote['description'] != null && (local['description'] == null || (remote['description'] as String).isNotEmpty)) {
